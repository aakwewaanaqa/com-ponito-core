...
namespace Ponito.Core.Threading.Classes
{
    ...
    /// </summary>
    public partial class LoopRunner
    {
        private const int SIZE = 16;

        private readonly Timing          timing         = Timing.Update;
        private readonly object          lockRunOrQueue = new();
        private readonly object          lockArray      = new();
        private readonly Queue<LoopItem> waits          = new(SIZE);
        private          LoopItem[]      items          = new LoopItem[SIZE];

        private bool isRunning { get; set; } = false;
        private int  tail      { get; set; } = 0;

        public void Add(LoopItem item)
        {
            lock (lockRunOrQueue)
            {
                if (isRunning)
